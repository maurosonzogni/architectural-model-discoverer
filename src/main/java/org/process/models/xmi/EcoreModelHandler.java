...
public class EcoreModelHandler {
    ...
    public void processModels(QueryModel eolBasedModelQuery, QueryModel javaBasedModelQuery) {
        ...
        for (String modelUri : this.uriModels) {
            try {
                ...
                System.out.println(
                        "\033[0;33m" + indexFile + "/" + this.uriModels.size() + " URI: " + modelUri + "\033[0m");
        ...
        if (this.processedDataFromModel.size() == 0)
            return;
    ...
    public void generateCSVFileFromProcessedModels(String name) {
        try {
            ...
            ////////// ////////////////////////////////////
            List<String> header = new ArrayList<>(Arrays.asList("model_name", "src_path", "conv_path", "src_ext",
                    "is_parsed", "is_sys_design", "sys_name"));
    ...
    List<Map<String, Object>> createDataSource() {
        ...
        return conversionLogs.stream().map(conversionLogModel -> {
            ...
            preData.put("doc_files",
                    String.join(", ", (Collection<String>) conversionLogModel.getOrDefault("docFiles", null)));
            return preData;
    ...
    void generateLegends(Config configObj) {
        try {
            ...
            dataSource.add(new String[] { "src_path",
                    "The global path where the model[source] was found in the file system" });
            ...
            dataSource.add(new String[] { "is_parsed",
                    "A boolean value that show if the model could be parsed or not, if it wasn't, model could be broken" });
            dataSource.add(new String[] { "is_sys_design",
                    "A boolean value that show if in the model[source] there was found a SystemInstance model" });
            dataSource.add(new String[] { "sys_name", "The name of the SystemInstance model found" });
            dataSource
                    .add(new String[] { "num_comp", "The number of component of the model: ComponentInstance found" });
            dataSource.add(
                    new String[] { "num_conn", "The number of connections of the model: ConnectionInstance found" });
            dataSource.add(new String[] { "size", "The sum of components and connection" });
            dataSource.add(new String[] { "udy",
                    "The Understandability: The number of connections between components divided by N^2-N, " +
            ...
            dataSource.add(new String[] { "coupling",
                    "Sum for every component of the number of in_features divided by the (out_features + in_features) : Features are connection to other components" });
            dataSource.add(new String[] { "cohesion",
                    "The computation of cohesion its returned as: e / (n(n-1))/2 where e and n are connections and components respectively" });
            dataSource.add(new String[] { "complexity", "The sum for every component of in_features + out_features" });
            dataSource.add(new String[] { "graph_density",
                    "The graph density is the ratio e / n, where e and n are connections and nodes respectively" });
            dataSource.add(new String[] { "graph_str_rep", "The graph string representation of the models" });
            dataSource.add(new String[] { "avg_shortest_path",
                    "The distance measure of a source to all other reachable destinations of a complex network used to model the software architecture" });
            dataSource.add(new String[] { "avg_deg_cent",
                    "The degree centrality of the graph based in components and connections" });
